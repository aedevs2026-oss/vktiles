...
/** Recently added catalogue lines (wall tiles, elevation, latest series). */
export function getNewArrivals(limit = 12) {
  ...
  const newCategories = new Set(["wall-tiles", "elevation-tiles"]);
  const fromNewLines = products.filter((p) => newCategories.has(p.category));

  const pool =
    fromNewLines.length >= limit
      ? fromNewLines
      : [...products].reverse();
